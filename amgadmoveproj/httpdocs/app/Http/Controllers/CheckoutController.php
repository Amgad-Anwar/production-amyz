        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('combined_order_id') != null) {
                ...
                if (class_exists($decorator)) {
                 ...
                 if ($request->payment_option == "paytabs")   {
                  ...
                  ->sendCart(10,Session::get('total_amount'),Auth::user()->name)
                  ->sendCustomerDetails(Auth::user()->name, Auth::user()->email, Auth::user()->phone, 'test', Auth::user()->city, 'Cairo', 'EG', '1234','100.279.20.'.rand(10,200))
        ...
        if ($carts && count($carts) > 0) {
            foreach ($carts as $key => $cartItem) {
               ...
               // dd($cartItem->product->name);
                //$percent = $cartItem->product->name;
                //amgad cat shipping
                //$new_cartItem_shipping_cost= $cartItem['shipping_cost'] ;
               // $cartItem['shipping_cost'] = $new_cartItem_shipping_cost ;
                $cartItem->save();



                $shipping += $cartItem['shipping_cost'];
                //end amgad cat shipping
            }
